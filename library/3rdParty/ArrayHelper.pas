...
type
  ...
  TArrayForEachCallbackFunc<T> = reference to function(var Value: T; Index: integer): Boolean;
  ...
  TArrayHelper = class helper for TArray
    ...
    // ForEach
    class procedure ForEach<T>(var Values: TArray<T>; const Callback: TArrayForEachCallback<T>); overload; static;
    class procedure ForEach<T>(var Values: TArray<T>; const Callback: TArrayForEachCallbackFunc<T>); overload; static;
...
class procedure TArrayHelper.ForEach<T>(var Values: TArray<T>; const Callback: TArrayForEachCallbackFunc<T>);
var
  I: Integer;
begin
  for I := Low(Values) to High(Values) do
    if not Callback(Values[I], I) then
      Break;
end;
